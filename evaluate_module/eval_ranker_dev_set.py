...
# Files paths
request_file_path = join(args.data_dir, "final", "clarQ_dev.tsv")
...
# try to read clarQ_dev_with_text_index.tsv
try:
    dev = pd.read_csv(join(args.data_dir, "final", "clarQ_dev_with_text_index.tsv"), sep='\t')
except:
    dev = pd.read_csv(request_file_path, sep='\t')
    unique_text = dev['text'].unique()
    unique_text_index = {text: i for i, text in enumerate(unique_text)}
    dev["text_index"] = dev["text"].apply(lambda x: unique_text_index[x])
    dev['question_id'] = dev['type'] + ':' + dev['keyword']
    # save dev file
    dev.to_csv(join(args.data_dir, "final", "clarQ_dev_with_text_index.tsv"), sep='\t', index=False)

# Runs bm25 for every query and stores output in file.
rerank_top_k = 30
if "dev_BM25" in listdir(run_file_path):
    print("BM25 run file already exists.")
else:
    with open(join(run_file_path, "dev_BM25"), 'w') as fo:
        for tid in dev['text_index'].unique():
            query = dev.loc[dev['text_index']==tid, 'text'].tolist()[0]
            print(query)
            bm25_ranked_list = bm25.get_top_n(stem_tokenize(query, True), bm25_corpus, n=rerank_top_k)
            print(bm25_ranked_list)
...
for epoch in tqdm(range(10, args.num_epochs+1)):
    ...
    # full reranking
    model_run_file_path = join(run_file_path, "dev_{}_{}_epoch{}".format(args.model_name.replace("/", "--"), args.negative_sampling_strategy, epoch))
    all_doc_ids = np.array(question_bank["question_id"].values)
    with open(model_run_file_path, 'w') as fo:
        for tid_idx, tid in enumerate(dev['text_index'].unique()):
