...
def convert_to_features(example_batch):
    example_batch_src = [prefix + ex for ex in example_batch['src']]
    input_encodings = tokenizer.batch_encode_plus(example_batch_src, 
...
def compute_metrics(eval_preds):
    ...
    # Evaluate CodeBLEU
    codebleu_result = codebleu_metric.compute(predictions=bleu_preds, references=bleu_labels)
    # Evaluate EM
    ...
    # Combine BLEU and EM
    result = {"sacrebleu": bleu_result["score"], 
              "exact_match": em_result["exact_match"],
              'codebleu': codebleu_result["CodeBLEU"],
              'ngram_match_score': codebleu_result['ngram_match_score'], 
              'weighted_ngram_match_score': codebleu_result['weighted_ngram_match_score'], 
              'syntax_match_score': codebleu_result['syntax_match_score'], 
              'dataflow_match_score': codebleu_result['dataflow_match_score']}
...
parser.add_argument("--top_k", default="", type=str)
...
if "code" in model_name:
    tokenizer = RobertaTokenizer.from_pretrained("Salesforce/{}".format(model_name))
else:
    tokenizer = T5Tokenizer.from_pretrained(model_name)
model = T5ForConditionalGeneration.from_pretrained(join(output_dir, "best_model"))
...
data_files = {"train": join(data_dir, "final", "{}_train.json".format(data_affix)),
              ...
              "test": join(data_dir, "final", "{}_test{}.json".format(data_affix, args.top_k))}
...
logger.info("*** Predict and Evaluate on the Test Set with {} ***".format(args.top_k))

predictions, label_ids, metrics = trainer.predict(dataset["test"], metric_key_prefix="test{}".format(args.top_k))
trainer.log_metrics("test", metrics)
trainer.save_metrics("test", metrics)

logger.info("*** Save predictions ***")
with open(join(save_dir, "preds{}.pkl".format(args.top_k)), "wb") as f:
